...
from solidium_webapp.ear_app import create_data
import solidium_webapp.ear_app.Ear_detecter
import os
import io
...
def Data_preprocess():
    path_1 = settings.EAR_URL+"Ear_image/Test/Before_preprocess/"
    path_2 = settings.EAR_URL+"Ear_image/Test/After_preprocess/"
    create_data.resize_pixel(path_1, path_1 + "After_resize_pixel/", 96, 96)
    create_data.change_color(path_1 + "After_resize_pixel/", path_2)
    create_data.class_Test_label(path_2, settings.EAR_URL+"Test_Data/")
...
def Fully_layer_1(previous_layer):
    ...
    F_L1_dropout = tf.nn.dropout(F_L1_relu, 1.0)
...
def Fully_layer_2(previous_layer):
    ...
    F_L2_dropout = tf.nn.dropout(F_L2_relu, 1.0)
...
def main():
    ...
    train_data = pd.read_csv(settings.EAR_URL+"Test_Data/Ear_test_data.txt", names=['image'])
    train_data.to_csv(settings.EAR_URL+'Ear_test_dataset.csv', header=False, index=False)
    Ear_test_dataset = pd.read_csv(settings.EAR_URL+'Ear_test_dataset.csv')

    Test_image_batch = Data_batch(settings.EAR_URL+"Test_Data/Ear_test_data.txt", len(Ear_test_dataset.index))
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, settings.EAR_URL+'checkpoint/ear_model')
    ...
    cnt_list =[YS, HJ, DJ, DH, JH]
    name_list = ['오윤석','임혜진','이동준','이동훈','손정효']

    cnt = np.argmax(cnt_list)

    #print("\n 예측횟수: [오윤석, 임혜진, 이동준, 이동훈, 손정효]\n")
    #print("\t--->", cnt_list)
    dic = {'오윤석': YS, '임혜진': HJ, '이동준': DJ, '이동훈': DH, '손정효': JH}
    #print("\n환영합니다. ", name_list[cnt], " 님")
    print(dic)
