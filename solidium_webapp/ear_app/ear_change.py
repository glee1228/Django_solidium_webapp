...
def Create_queue(text_file_name, num_epochs=None):
    ...
    train_data.to_csv(settings.EAR_URL+'Ear_dataset.csv', header=False, index=False) # 나중에 index 수를 얻기위해 하나 생성
...
if __name__ == "__main__":
    ...
    #######################
    image_batch, label_batch, file_batch = Data_batch(settings.EAR_URL+"Train_Data/Ear_train_data.txt")
    Ear_dataset = pd.read_csv(settings.EAR_URL+'Ear_dataset.csv')
    ...
    for epoch in range(FLAGS.training_epochs):
        ...
        validate_image_batch, validate_label_batch, validate_file_batch = Data_batch_Validation(settings.EAR_URL+"Train_Data/Ear_validation_data.txt", 1000)
        coord = tf.train.Coordinator()
        threads = tf.train.start_queue_runners(sess=sess, coord=coord)
    ...
    save_path = saver.save(sess, settings.EAR_URL+"checkpoint/ear_model")
